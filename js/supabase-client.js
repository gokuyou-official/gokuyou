(function () {
  try {
    window.supabaseClient = supabase.createClient(
      window.GOKUYOU_CONFIG.SUPABASE_URL,
      window.GOKUYOU_CONFIG.SUPABASE_KEY
    );
    window.gAuth = {
      signIn: function (email) {
        return window.supabaseClient.auth.signInWithOtp({
          email: email,
          options: { emailRedirectTo: 'https://gokuyou-official.github.io/gokuyou/pages/auth-callback.html' }
        });
      },
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      window.supabaseClient.auth.getSession().then(function (result) {
        ...
        navLink.innerHTML = session
          ? '<a href="/gokuyou/pages/profile.html">マイページ</a>'
          : '<a href="/gokuyou/pages/login.html">ログイン</a>';
      }).catch(console.error);
    });
